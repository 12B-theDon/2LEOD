...
# Load the 36-dimensional config and let it adjust the base config before use.
_CONFIG_36_PATH = Path(__file__).with_name("36_config.py")
_CONFIG_36_SPEC = importlib.util.spec_from_file_location(
    "config36", _CONFIG_36_PATH
)
if _CONFIG_36_SPEC is None or _CONFIG_36_SPEC.loader is None:
    raise RuntimeError(f"Unable to import 36_config from {_CONFIG_36_PATH}")
config36 = importlib.util.module_from_spec(_CONFIG_36_SPEC)
_CONFIG_36_SPEC.loader.exec_module(config36)

TARGET_BG_RATIO = config36.TARGET_BG_RATIO
THRESHOLDS = config36.THRESHOLDS
ODOM_TARGET_COLUMNS = config36.ODOM_TARGET_COLUMNS

from data_utils import load_dataset
from logger import get_logger
from models import build_classifier_pipeline, build_regressor

CLASSIFIERS_TO_RUN = ["logreg", "svm", "xgb"]
DEFAULT_MODEL_OUT_TEMPLATE = "models/36_opponent_bundle_{clf}.joblib"
...
def _resolve_model_out_path(template: str, classifier_type: str) -> Path:
    if "{clf}" in template:
        return Path(template.format(clf=classifier_type))
    base = Path(template)
    suffix = base.suffix
    name = f"{base.stem}_{classifier_type}"
    if suffix:
        name += suffix
...
def _save_accuracy_diagnostic_plot(
    ...
    fig_path = stats_dir / f"36_{prefix}_accuracy.png"
...
def _save_roc_pr_plot(
    ...
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    stats_dir.mkdir(parents=True, exist_ok=True)
    fig_path = stats_dir / f"36_{prefix}_roc_pr_score.png"
    fig.savefig(fig_path, dpi=150)
    plt.close(fig)
...
def _train_classifier(
    ...
    stats_dir = stats_root / prefix
    stats_dir.mkdir(parents=True, exist_ok=True)
    logger.info(f"[train][classifier] Type={classifier_type}")
    logger.info(
        f"[train][data] Training positives={pos_train}, negatives={neg_train}, "
    ...
    logger.info(
        f"[train][{prefix}] Confusion matrix:\n{confusion_matrix(y_class_test, y_pred_test)}"
    )
    logger.info(
    ...
    _save_score_distribution_plot(
        ...
        y_class_train,
        best_threshold,
        stats_dir / f"36_{prefix}_score_dist_train.png",
        score_label,
    ...
    _save_score_distribution_plot(
        ...
        y_class_test,
        best_threshold,
        stats_dir / f"36_{prefix}_score_dist_test.png",
        score_label,
    ...
    model_path = _resolve_model_out_path(args.model_out, classifier_type)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the opponent detection bundle with the 36-dimensional feature set."
    )
    parser.add_argument(
        "--model-out",
        "-m",
        type=str,
        default=DEFAULT_MODEL_OUT_TEMPLATE,
        help="Where to save the trained bundle (default: %(default)s).",
    )
    parser.add_argument(
        "--fig-root",
        type=Path,
        default=Path("figs"),
        help="Root directory for evaluation figures (default: %(default)s).",
    )
    parser.add_argument(
        "--classifiers",
        nargs="+",
        choices=CLASSIFIERS_TO_RUN,
        default=CLASSIFIERS_TO_RUN,
        help="Classifier pipelines to train sequentially (default: %(default)s).",
    )
...
def main() -> None:
    ...
    stats_root = args.fig_root / "36_train_eval"
