...
So we can say that if we have 360-degree distance data, then width will be 360 (360 number of bins) and height will be distances. So the θ-degree value is mapped to a column vector, and the height of this column vector is the maximum lidar scan distance (detection range). When the lidar has an obstacle at 2 m, the values from 0 m to 2 m increase in intensity, and no value is assigned above 2 m up to the maximum lidar scan distance. Black (0) lies at y = 0, and white (255) lies at y equal to the detected distance. The values between are smoothly increased (calculated automatically). Like an RGB channel, we can choose three channel values: (1) Distance, (2) Density (how many points are in that range), and (3) Delta (current distance minus previous distance). Then we can get an RGB color map. Since we have labels isWall, isOpponent, isFree, and isStatic, each column has a label. 
...
- **Input**: For each frame we consider the point set P^(i) = {(x_k, y_k)}_{k=1}^{N_i} obtained by mapping the 360-range vector x^(i) into Cartesian coordinates using (x_k, y_k) = (r_k cos θ_k, r_k sin θ_k).
- **Process**: We cluster the angles into two groups and fit two linear models y = a_j x + b_j (or x = c_j for near-vertical cases) for j = 1, 2 by ordinary least squares. Each fit produces slope/intercept pairs (a_j, b_j), directional vectors d_j, and line centroids (x̄_j, ȳ_j).
- **Output**: For every frame the module emits:
  1. Two line models ℓ_1 and ℓ_2 with (a_j, b_j), direction, support counts, and a vertical flag.
  2. `point_info` for each scan containing `distance_to_line`, `line_id`, and a boolean `is_wall` based on the threshold τ: isWall_k = min_j dist_j(k) ≤ τ where dist_j(k) denotes the perpendicular residual.
...
- **Input**: Every training sample is the original 360-dimensional vector x^(i) in ℝ^360 fed directly to the classifier.
- **Process**: The module keeps weight matrix W in ℝ^(4×360) and bias vector b in ℝ^4, computes logits z_j = W_j · x^(i) + b_j, evaluates softmax probabilities p_j = exp(z_j)/Σ_{k=1}^4 exp(z_k), and minimizes cross-entropy loss -log p_label via SGD.
- **Output**: After training it exposes:
  1. `predict_proba(x)` yielding p_1,…,p_4,
  2. `predict(x) = argmax_j p_j`, and
  3. Serialized weights/biases through `to_dict()` for checkpointing.

#### `models/ensemble.py`
- Combines the softmax probabilities with the linear regression distance signal. The `ensemble_prediction` helper boosts the wall class probability by a tunable weight before voting after rescoring via wall_confidence(d, τ) = 1 - min(d, τ)/τ, which rescales the closest residual d into a [0,1] score.
